...
Reference: `googlecolab/google-colab-cli`. colab uses it as a feature reference only; no code is copied or vendored.

| google-colab-cli feature | colab command | Status | Note |
|---|---|---|---|
...
| `drivemount` | `colab fs drive mount` | partial | Uses kernel execution and staged errors. Full Drive credential propagation is documented as a remaining gap until live-tested. |
| `install` | `colab run pip install` | implemented | Old `run install` is a hidden migration alias. |
| `log` | `colab log` | surface added | Reports persisted colab logs when available; does not invent missing remote logs. |
...
Compatibility aliases stay hidden from normal help. New docs use only the colab command spaces.

## Interactive execution mapping

| google command | colab command | Transport | Note |
|---|---|---|---|
...
| `colab console` | `colab run shell` | Colab `/colab/tty` PTY websocket | colab does not assume `/api/terminals` exists for shell. Piped stdin sends `exit\n` on EOF. |
...
| `colab drivemount` | `colab fs drive mount` | Kernel execution | Runs `google.colab.drive.mount()` in the attached Colab kernel. The Rust credential-propagation hook is not yet claimed complete. |
| `colab log` | `colab log` | Local colab log/export surface | Does not invent unavailable remote logs. |
| `colab restart-kernel` | `colab session kernel restart --yes` | Jupyter kernel API | Confirmation-gated because it interrupts work. |
| kernel picker | `colab session kernel list/select/specs/current` | Jupyter kernel/session/kernelspec APIs | colab caches detected language per local session. |
